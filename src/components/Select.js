...
const StyledSelect = styled.select`
  ...
  background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='#b1b1b1'><polygon points='0,0 20,0 10,10'/></svg>");
...
const Select = ({ handleChange, name, values, value }) => (
  <StyledSelect name={name} value={value} id={name} onChange={handleChange}>
    ...
    {values.sort(sortNameAlpha).map(value => (
      <option value={`${value.id}|${value.name}`} key={value.id}>{value.name}</option>
    ))}
  </StyledSelect>
);
